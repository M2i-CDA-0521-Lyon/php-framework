...
<div class="container">
    ...
    <main>
        ...
        <h2>Posté le <?= $topic->getDate()->format('d/m/Y h:i') ?> par <?= $topic->getAuthor()->getUsername() ?></h2>

        <ul id="message-list">
            <?php foreach($topic->getMessages() as $message) { ?>
                <li>
                    <h3>Par <?= $message->getAuthor()->getUsername() ?> le <?= $message->getDate()->format('d/m/Y h:i') ?></h3>
        ...
        <form action="/message/<?= $topic->getId() ?>" method="POST">
